...
if (oInfo.fighting) {
	if (!sequence_cooldown) {
		oBackground.txt_right = string_char_at(oInfo.sequence2, sequence_spot+1) + oBackground.txt_right;
		
		if (string_char_at(oInfo.sequence2, sequence_spot+1) == "1") { ////////////111111111111111111111
		...
		} else if (string_char_at(oInfo.sequence2, sequence_spot+1) == "2") { ///////22222222222222222222222222
		...
		} else if (string_char_at(oInfo.sequence2, sequence_spot+1) == "3") { //3.
		...
		if (sequence_spot == len(oInfo.sequence2)) {
			sequence_spot = 0;
		}
	}
}

draw_sprite_ext(sRock, 0, x, y, 1, 1, angle, merge_colour(oInfo.colour2, c_red, hurt/10), 1);
