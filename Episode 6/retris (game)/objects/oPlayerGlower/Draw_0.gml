...
if (player.powerup_triggering == 0) {
	...
	if (player.red_selected > 0) {
		...
		var xx = player.x;
		var yy = player.y+player.powerup_y*oInfo.square_size;
		var xx2 = xx + oInfo.w;
		var yy2 = yy + player.red_lines*oInfo.square_size;
		
		var xx3 = guy.x;
		var yy3 = guy.y+oInfo.h - player.red_lines*oInfo.square_size;
		var xx4 = xx3 + oInfo.w;
...
if (blue_lifespan > 30) {
	...
	var spotx = player.x+player.shake_x;
	var spoty = player.y+player.shake_y;
	var w = power(l, 1.5)/50;
	// p1  p2
	//       
	// p4  p3
	var p1 = [spotx, spoty+(oInfo.h-amount*oInfo.square_size)];
	var p2 = [spotx+oInfo.w, spoty+(oInfo.h-amount*oInfo.square_size)];
	var p3 = [spotx+oInfo.w, spoty+oInfo.h];
	var p4 = [spotx, spoty+oInfo.h];
