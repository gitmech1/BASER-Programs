...
function len(thing){ //finds the length of different data types (python reference)
...
function msg(words){ //show_debug_message but shortened
	show_debug_message(words);
}

function sfx(index, priority=1000, loop=false, gain=1, offset=0, pitch=1){ //shortened audio_play_sound
	return audio_play_sound(index, priority, loop, gain, offset, pitch);
}
...
function image_scale(num) { //image_xscale = num; image_yscale = num;
	image_xscale = num;
	image_yscale = num;
}
...
function round_dp(num, dp) { //rounding with decimal place precision
	return round(num*power(10, dp))/power(10, dp);
}

function map_value(value, p1, p2, np1, np2) {
	var lerped = reverse_lerp(p1, p2, value);
	var new_value = lerp(np1, np2, lerped);
	return new_value;
}

function map_value_clamp(value, p1, p2, np1, np2) {
	return clamp(map_value(value, p1, p2, np1, np2), min(np1, np2), max(np1, np2));
}

function sa(str, ind, replacement){ //replace guy in string with new guy
...
function string_char_range(str, lower, upper) { //returns string within range (python: str[lower:upper]) (includes lower but not upper)
	var new_str = "";
...
function array_contains_range(array, x1, x2) { //check if array contains entire range of numbers
	for (var i = min(x1, x2); i <= max(x1, x2); i++) {
		if !(array_contains(array, i)) {
			return false;
		}
	}
	return true;
...
function weighted_random(weights) { //pass in array of weights [1, 2, 3], returns index
...
function frames_to_time(frames, fpss) {
	var seconds = string(floor(frames/fpss) % fpss);
	var minutes = string(floor(frames/fpss/fpss) % 99);
...
function array_range(lower, upper) { //includes lower but not upper
	var da_array = [];
	for (var i = lower; i < upper; i++) {
		array_push(da_array, i);
	}
	return da_array;
}
...
function array_find(array, value) { //find index of value
...
function array_replace(array, to_replace, replacement) {
	var new_array = array_clone(array);
	for (var i = 0; i < len(new_array); i++) {
		if (new_array[i] == to_replace) {
...
function array_clone(array) { //creates new pointer (wtf are pointers)
	var new_array = [];
	array_copy(new_array, 0, array, 0, len(array));
	return new_array;
}

function array_create_clone(array, amount) { //duplicates an array set amount of times into new array
	var new_array = [];
	for (var i = 0; i < amount; i++) {
		array_push(new_array, array_clone(array));
	}
	return new_array;
}

function array_create_2d(iSize, jSize, value=0) { //array_create but layered as to not have pointer issues (i dont understand it either)
    var result = array_create(iSize);
    for (var i = iSize-1; i >= 0; --i) {
        result[i] = array_create(jSize, value);
    }
    return result;
} //frostycat u r the best

function array_contains_array(array, value) {
	for (var i = 0; i < len(array); i++) {
		if (array_equals(array[i], value)) {
			return true;
		}
	}
	return false;
...
function array_part(array, lower, upper) { //return only specified part of array (includes lower but not upper)
	var new_array = [];
	for (var i = max(lower, 0); i < min(upper, len(array)); i++) {
		array_push(new_array, array[i]);
	}
	return new_array;
}

function array_function(array, func) { //use a function on every item in array
	var new_array = [];
	for (var i = 0; i < len(array); i++) {
		array_push(new_array, func(array[i]));
	}
	return new_array;
}

function array_min(array) { //finds min value in array
	var lowest = infinity;
	for (var i = 0; i < len(array); i++) {
		if (array[i] < lowest) {
			lowest = array[i];
...
function array_max(array) { //finds max value in array
...
function array_sum(array) { //sums all guys in array
	var total = 0;
	for (var i = 0; i < len(array); i++) {
		total += array[i];
	}
...
function array_sign(array, dimensions) { //signifies everything in array
...
function array_lerp(array1, array2, amount) {
	var new_array = [];
	for (var i = 0; i < min(len(array1), len(array2)); i++) {
		array_push(new_array, lerp(array1[i], array2[i], amount));
	}
	return new_array;
}
...
function array_combine(arrays, positions) { //structure eg; array_combine([[1, 2, 3, 4], [5, 6, 7]], [0, 2]);
...
function array_count(array, value) { //counts how many times a value shows up in an array
	var count = 0;
	for (var i = 0; i < len(array); i++) {
		if (array[i] == value) {
			count++;
...
function array_combine_2d(arrays, positions) { //assumes everything is rectangular, structure eg; array_combine_2d([[[1, 2, 3], [3, 2, 4], [5, 1, 1]], [[5, 6], [7, 9]]], [[0, 1], [2, 3]]);
...
function draw_center_rectangle_color_simple(xpos, ypos, w, h, col, outline=false) {
	draw_rectangle_color(xpos-w/2, ypos-h/2, xpos+w/2, ypos+h/2, col, col, col, col, outline);
}
...
function draw_border_rectangle(x1, y1, x2, y2, width, bordercol, insidecol) {
	...
	draw_rectangle_color(x1, y1, x2, y2, bordercol, bordercol, bordercol, bordercol, false);	
	draw_rectangle_color(x1+width, y1+width, x2-width, y2-width, insidecol, insidecol, insidecol, insidecol, false);
}
...
function draw_line_edges(x1, y1, x2, y2, w, col1, col2) { //line with circle tip
...
function surface_clear() { //removes all of surface gpu style
...
function line_point(px, py, x1, y1, x2, y2, buffer=0.1) { //check if point on line, with buffer as lenience
...
function rectangle_point(px, py, x1, y1, x2, y2) { //check if point in rectangle
    if (x1 > x2) {
		var temp = x1;
		x1 = x2;
        x2 = temp;
...
function circle_point(px, py, cx, cy, r) { //check if point in rectangle
...
function circle_circle(cx1, cy1, r1, cx2, cy2, r2) { //check if circle hitting circle
...
function circle_line(cx, cy, r, x1, y1, x2, y2) { //check if line hitting circle (weird math stuff)
...
function triangle(n, factor=1) { //triangle wave
...
function dot_product_more(x1, y1, x2, y2, px, py) { //i didnt do spec
    var line_length = pythag(x1, y1, x2, y2);
...
function dfs(map, xx, yy) { //depth first search (uses weird ds_list stuff)
	var to = 2;
	var from = map[# xx, yy];
...
// PHYSICS ========================================================================================================================= (just suvat rn)

function solve_v(u, s, a) {
	return sqrt(sqr(u) + 2*a*s);
}

function solve_v_2(u, a, t) {
	return u + a*t;
}

function solve_s(u, t, a) {
	return u*t + 0.5*a*power(t, 2);
}

function solve_t(v, u, a) {
	return (v-u)/a;
}

function solve_t_2(u, s, a, v=0) {
	if (u == 0) {
		return sqrt(2*s/a);
	} else {
		if (s == 0) {
			return solve_t(v, u, a);
		} else {
			return solve_t(solve_v(u, s, a), u, a);
		}
	}
}
...
function no_ease(n){ //only useful to pass into functions
	if (n < 0) {
		return 0;
	} else if (n > 1) {
		return 1;
...
function ease_in(n, p=2){ //slow start
	if (n < 0) {
		return 0;
	} else if (n > 1) {
		return 1;
...
function ease_out(n, p=2){ //slow end
	if (n < 0) {
		return 0;
	} else if (n > 1) {
		return 1;
...
function ease_in_out(n, p=1.7){ //slow both
	if (n < 0) {
		return 0;
	} else if (n > 1) {
		return 1;
...
function ease_bounce(n, a, u, recoil, limit=0.01, show_bounces=false) { //boioioiong
	//n is frame iteration
	//a is acceleration
	//u is initial velocity
	//recoil is bounce back force
	//limit is what velocity the ball stops to maintain speed
	//show_bounces just includes a frame of the ball at lowest position when it bounces
	
	a = abs(a);
	
	var s = 1;
	var v = solve_v(u, s, a);
	var t = solve_t_2(u, s, a, v);
	
	if (t > n) {
		return 1-solve_s(u, n, a);
	}
		...
		var true_t = t-(time_elapsed-n);
		
		if (true_t < 1) && (show_bounces) {
			return 0;
		} else {
			return -solve_s(u, true_t, a);
		}
	}
}

function ease_bounce_old(n) { //bounces in
	var nn = n*0.844;
	if (nn < 0) {
		return 0;
	} else if (nn < 0.4) {
...
function elastic_ease(n, stretch=3) { //shloings in
...
function generate_maze(sizex, sizey) { //generates a maze (school code)
...
function perlin_noise(_x, _y = 100.213, _z = 450.4215) { //generates perlin_noise in 1, 2 or 3 dimensions (dont use integers (only decimal), idk why) (_y and _z are random they dont matter)
	
	//https://www.youtube.com/watch?v=ms1wczeXAT0 for explanation
...
function generate_perlin_noise(spread, xsize, ysize=0, zsize=0) { //larger the spread, wider the display (format[xx][yy][zz])
	var map = [];
	var xseed = random_range(0, 1000);
	var yseed = random_range(0, 1000);
	var zseed = random_range(0, 1000);
	...
	if (zsize > 0) {
		for (var xx = 0; xx < xsize; xx++) {
			...
			for (var yy = 0; yy < ysize; yy++) {
				var temp1 = [];
				for (var zz = 0; zz < zsize; zz++) {
					array_push(temp1, perlin_noise(xseed+xx/spread, yseed+yy/spread, zseed+zz/spread));
				}
				array_push(temp2, temp1);
			}
			array_push(map, temp2);
		}
	} else if (ysize > 0) {
		for (var xx = 0; xx < xsize; xx++) {
			var temp1 = [];
			for (var yy = 0; yy < ysize; yy++) {
				array_push(temp1, perlin_noise(xseed+xx/spread, yseed+yy/spread));
			}
			array_push(map, temp1);
		}
	} else {
		for (var xx = 0; xx < xsize; xx++) {
			array_push(map, perlin_noise(xseed+xx/spread));
		}
	}
...
function get_save(reciept=false) { //shows what is saved (put it in chat or return it)
	var struct = pull();
	
	if (reciept) {
		var names = variable_struct_get_names(struct);
		msg("== ALL SAVE VARIABLES ==========================================================");
		msg("");
		for (var i = 0; i < len(names); i++) {
			msg("- " + names[i] + ":");
			msg("  - " + string(struct[$ names[i]]));
			msg("");
		}
		if (len(names) == 0) {
...
function push(keyword, data, reciept=false) { //add something to save
	//get save stuff
	...
	if (reciept) {
		if (updated) {
			msg("- " + string(keyword) + " has been updated");
		} else {
			msg("- " + string(keyword) + " has been added");
		}
	};
}

function pull(keyword="NOTHING", reciept=false) { //get stuff from save (not inputting anything returns everything saved)
	if (file_exists("savedgame.save")) {
		...
		if (keyword == "NOTHING") {
			if (reciept) {
				msg("- returned EVERYTHING");
			}
			return struct; //full struct
		} else {
			if (struct_exists(struct, keyword)) { //check if it actually exists
				if (reciept) {
					msg("- returned " + string(keyword));
				}
				return struct[$ keyword]; //keyword only ($ symbol used to convert string keyword to real? keyword)
			} else {
				if (reciept) {
					msg("- " + string(keyword) + " wasnt in the save");
				}
				return [];
			}
		}
...
function snap(keyword, reciept=false) { //remove saved item
	if (file_exists("savedgame.save")) {
		//make into data mode
		var struct = pull();
		...
		if (struct_exists(struct, keyword)) {
			...
			if (reciept) {
				msg("- " + string(keyword) + " has been snapped.");
			}
		} else {
			if (reciept) {
				msg("- " + string(keyword) + " wasnt in the list, thus no snappage");
			}
		}
...
function snap_all(reciept=false) {
	var everything = get_save(false);
	var names = variable_struct_get_names(everything);
	if (len(names) != 0) {
		for (var i = 0; i < len(names); i++) {
			snap(names[i], reciept);
		}
		if (reciept) {
			msg("- EVERYTHING IS GONE");
		}
	} else {
		msg("- there aint nothing to snap");
	}
}

function in_save(keyword) { //check whether keyword is in save
	if (file_exists("savedgame.save")) {
		var struct = pull();
...
function ctc(txt){ //convert to cool (from el school projecto)
...
function f3(title, array, xpos, ypos, halign=fa_left, spacing=25, border=25) { //[[name, value], [name, value], [name, value], [name, value], [name, value], [name, value], [name, value], ...]
	//generating words
	var all_words = [];
	for (var i = 0; i < len(array); i++) {
		array_push(all_words, string(array[i]));
	}
	//finding horizontal length
	var max_length = string_width(title);
	for (var i = 0; i < len(all_words); i++) {
		max_length = max(max_length, string_width(all_words[i]));
	}
	//drawing
	//box
	draw_set_alpha(0.5);
	if (halign == fa_left) {
		draw_rectangle_color_simple(xpos, ypos, xpos+max_length+border*2, ypos+spacing*(len(all_words)+1)+border*2, c_black);
		var spot = xpos+border;
	} else if (halign == fa_center) {
		draw_rectangle_color_simple(xpos-(max_length+border*2)/2, ypos, xpos+(max_length+border*2)/2, ypos+spacing*(len(all_words)+1)+border*2, c_black);
		var spot = xpos;
	} else if (halign == fa_right) {
		draw_rectangle_color_simple(xpos-max_length-border*2, ypos, xpos, ypos+spacing*(len(all_words)+1)+border*2, c_black);
		var spot = xpos-border;
	}
	draw_set_alpha(1);
	//text
	...
	draw_text(spot, ypos+border+spacing, title);
	for (var i = 0; i < len(all_words); i++) {
		draw_text(spot, ypos+border+spacing*(i+2), all_words[i]);
	}
	
	return [max_length+border*2, spacing*(len(all_words)+1)+border*2]; //width and height of box
}

function f3_strings(title, variables, xpos, ypos, halign=fa_left, spacing=25, border=25) { //puts it in 'dictionary' form
	//generating words
	var all_words = [];
	for (var i = 0; i < len(variables); i++) {
		array_push(all_words, string(variables[i][0]) + ": " + string(variables[i][1]));
	}
	return f3(title, all_words, xpos, ypos, halign, spacing, border);
}

function f3_2d_array(title, array_2d, xpos, ypos, delim=" ", halign=fa_left, spacing=25, border=25) { //converts every array within the string form
	//generating words
	var all_words = [];
	for (var i = 0; i < len(array_2d); i++) {
		array_push(all_words, string_join_ext(" ", array_2d[i]));
	}
	return f3(title, all_words, xpos, ypos, halign, spacing, border);
}

function string_start_of_array(array, size) { //only shows end of array in string form
	return string_char_range(string(array), 0, min(len((string(array))), size));
}

function string_end_of_array(array, size) { //only shows end of array in string form
	return string_char_range(string(array), len(string(array))-min(size, len(string(array))), len(string(array)));
}


// OTHER =========================================================================================================================

function get_datetime() { //returns date and time (string)
	return date_datetime_string(date_current_datetime());
}

function get_date() { //returns date (string)
	return date_date_string(date_current_datetime());
}

function get_time() { //returns time (string)
	return date_time_string(date_current_datetime());
}

function get_keyboard_check() { //get all letters
...
function get_keyboard_pressed() { //get all letters pressed
	var letters = ["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P",
					"A", "S", "D", "F", "G", "H", "J", "K", "L", 
					"Z", "X", "C", "V", "B", "N", "M"];
	var pressed = [];
...
function get_keyboard_released() { //get all letters released
	var letters = ["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P",
					"A", "S", "D", "F", "G", "H", "J", "K", "L", 
					"Z", "X", "C", "V", "B", "N", "M"];
	var pressed = [];
...
function bin_to_num(bin) { //binary to number (binary is string)
	var total = 0;
...
function num_to_bin(num) { //number to binary (binary is string)
...
function array_make(value, amount) { //this is just array_create
	var list = [];
	for (var i = 0; i < amount; i++) {
