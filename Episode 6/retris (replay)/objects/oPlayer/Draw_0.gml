...
var _x = oInfo.w_extra;
var _y = oInfo.h_extra;


surface_set_target(everything_surface);
surface_clear();


draw_sprite(sBorder, 1+colour_id*3, _x+shake_x, _y+shake_y); //back of border
...
if (len(voided_garbage)) {
	...
	for (var i = 0; i < len(voided_garbage); i++) {
		...
		for (var j = 0; j < len(voided_garbage[i][0]); j++) {
			...
			if (guy != 0) {
				draw_sprite_ext(sBrick, guy-1, _x+oInfo.square_size*j, _y-(len(voided_garbage)-i)*oInfo.square_size, 1, 1, 0, -1, transparency);
			}
		}
	}
...
if (len(height_map)) { //boing down
	for (var i = 0; i < oInfo.grid_w; i++) {
		...
		draw_surface_part(grid_surface, xx, 0, oInfo.square_size, oInfo.h-guy.red_lines*oInfo.square_size, _x+xx+shake_x, _y+yy+shake_y);
		draw_surface_part(grid_surface, xx, oInfo.h-bottom_height, oInfo.square_size, bottom_height, _x+xx+shake_x, _y+oInfo.h-bottom_height+shake_y);
	}
} 
else if (dead <= 90) {
	draw_surface(grid_surface, _x+shake_x, _y+shake_y); //normal
} 
else { //withering away
	var shift = (dead-90) * (oInfo.h/30);
	if (shift < oInfo.h) {
		draw_surface_part(grid_surface, 0, shift, oInfo.w, oInfo.h-shift, _x+shake_x, _y+shake_x+shift);
	}
}

if (hover) && !(dead) && !(pause) { //hover piece
	draw_piece(piece, rotation, distance, hover_y, 1, _x, _y);
}
...
if (powerup_triggering == 1) { //done as sender
	...
	if (powerup_lifespan >= 15) {
		for (var i = 0; i < len(block_order); i++) { //[old block pos, victim sending, victim block spot, colour, lifespan]
			...
			if (g[3] != 0) {
				draw_sprite(sBrick, block_id_to_index(g[3]), _x+shake_x+g[0][0]*oInfo.square_size, _y+shake_y+g[0][1]*oInfo.square_size-leave);
			}
		}
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if (pointer_player.powerup_lifespan >= 15) {
		for (var i = 0; i < len(pointer_spots); i++) { //[old block pos, victim sending, victim block spot, colour, lifespan]
			...
			if (g[3] != 0) {
				draw_sprite(sBrick, block_id_to_index(g[3]), _x+shake_x+g[2][0]*oInfo.square_size, _y+shake_y+g[2][1]*oInfo.square_size-enter);
			}
		}
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//piece coming next
if (dead <= 65) {
	draw_sprite(sPieceNext, piece_next, _x+shake_x, _y+shake_y);
}
...
if (dead <= 65) {
	...
	var xspot = _x+xraw*oInfo.square_size + shake_x;
	var yspot = _y+(oInfo.grid_h-the_height+2)*oInfo.square_size + shake_y;
	if (between(xraw, 0, 2)) {
		var yrod = _y-128 + shake_y;
	} else if (between(xraw, 2, 5.5)) {
		var yrod = _y-140 + shake_y;
	} else if (between(xraw, 5.5, 11)) {
		var yrod = _y-147 + shake_y;
	}
...
if (dead >= 215) {
	...
	draw_sprite_ext(sTombs, 0, _x+oInfo.w/2+shake_x, _y+930+shift+shake_y, 1, 1, 0, colour, 1);
	draw_sprite(sTombs, placement, _x+oInfo.w/2+shake_x, _y+930+shift+shake_y);
}

//more rock (this is going to be hell to animate
if (between(dead, 110, 230, false)) {
	draw_sprite(sFishRod2, 0, _x+oInfo.w/2, _y+1081);
	draw_sprite(sFishRock2, colour_id, _x+oInfo.w/2+random_range(-1, 1), _y+1081+random_range(-1, 1));
}

//drawing temporary times
if !(dead) {
	//PIECE THAT IS BEING DROPPED
	draw_piece(piece, rotation, distance, the_height, 0, _x, _y); //piece dropping
	//warnings
	if (len(garbage_recieving)) {
		for (var i = 0; i < len(garbage_recieving); i++) {
			...
			draw_sprite_part_ext(sWarning, 0, 0, 0, 400, 40*(len(guy[0])+1), _x+shake_x, _y+shake_y+oInfo.h-40*(len(guy[0])+1), 1, 1, guy[2], transparency);
		}
	}
}

if (dead <= 65) {
	draw_sprite(sBorder, 0+colour_id*3, _x+shake_x, _y+shake_y); //border no planks
} else {
	draw_sprite(sBorder, 2+colour_id*3, _x+shake_x, _y+shake_y); //border planked up
}


if (trash_target_timer) && !(dead) {
	var xx = _x+oInfo.w/2 + random_range(-trash_target_shake, trash_target_shake);
	var yy = _y+oInfo.h/3 + random_range(-trash_target_shake, trash_target_shake);
...
var spot = find_real_pos(-_x, -_y);
var xxx = spot[0] + (map_value(x+oInfo.w/2, 300, 1530, -2, 0))*615*(size-1);
var yyy = spot[1];
draw_surface_ext(everything_surface, xxx, yyy, size, size, 0, -1, 1);
...
for (var i = 0; i < len(shenanigan_senders); i++) {
	...
	var str = oInfo.colour_names[oInfo.colour_conversion[guy[1]]] + " is targeting you.";
	draw_border_text(x+oInfo.w/2, oC.cy1+spot, str, -1, 99999, 1, 1, 3, 0, oInfo.colours[guy[1]][0], oInfo.colours[guy[1]][1], 1, true);
...
/*var spot = find_real_pos(0, 0);
draw_circle_color(spot[0], spot[1], 20*size, make_color_hsv((lifespan*2)%255, 255, 255), make_color_hsv((lifespan*2+20)%255, 255, 255), false);*/
