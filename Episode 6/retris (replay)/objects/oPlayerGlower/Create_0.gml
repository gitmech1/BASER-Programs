...
function draw_edge_thingy(lifespano, col_id, wane=0) {
	var raw_in = ease_out(lifespano/60)*5*map_value(sin(degtorad(lifespano*2.5+180)), -1, 1, 0.5, 1.5) * (1-ease_in(wane/30, 4)) * (oC.cx2-oC.cx1)/2560;
	var in = floor(raw_in);
	...
	draw_rectangle_color_simple(oC.cx1, oC.cy1, oC.cx2, oC.cy1+in, col);
	draw_rectangle_color_simple(oC.cx1, oC.cy1+in+1, oC.cx1+in, oC.cy2-in-1, col);
	draw_rectangle_color_simple(oC.cx1, oC.cy2-in, oC.cx2, oC.cy2, col);
	draw_rectangle_color_simple(oC.cx2-in, oC.cy1+in+1, oC.cx2, oC.cy2-in-1, col);
	//anti aliasing line
	draw_set_alpha(alpha*in_left);
	draw_rectangle_color_simple(oC.cx1+in+1, oC.cy1+in+1, oC.cx2-in-1, oC.cy2-in-1, col, true);
