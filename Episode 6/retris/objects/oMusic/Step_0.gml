...
if (audio_is_playing(song_instance)) {
	...
	//actually set gain
	audio_sound_gain(song_instance, fade_in_instance_vol*volume*volume2, 0); //0 means per step
}
...
for (var i = 0; i < len(fade_out_instances); i++) {
	...
	audio_sound_gain(fade_out_instances[i], fade_out_instance_vol[i]*volume*volume2, 0);
