...
function array_push_list(array, values){ //adds list of values to array
	for (var njldfs = 0; njldfs < len(values); njldfs++) {
		array_push(array, values[njldfs]);
	}
}

function array_find(array, value) { //finds index of specific value in array
	for (var i = 0; i< len(array); i++) {
		if (string(array[i]) == string(value)) { //convert to strings because arrays are stupid
			return i;
		}
	}
	return -1;
...
function check_array(array, val){ //checks if value in array
...
function frames_to_time(frames, game_fps=60) {
	var seconds = string(floor((frames/game_fps)%60));
	var minutes = string(floor((frames/game_fps/60)%60));
...
function draw_outline_rectangle(x1, y1, x2, y2, width, col) {
	draw_line_width_color(x1, y1, x2, y1, width, col, col);
	draw_line_width_color(x1, y2, x2, y2, width, col, col);
	draw_line_width_color(x1, y1, x1, y2, width, col, col);
	draw_line_width_color(x2, y1, x2, y2, width, col, col);
}
...
function clear_surface(surface=-1) {
	if (surface == -1) {
...
function get_polygon_internal_angle(p1, p2) {
	return (pi + arctan2((p1[0]*p2[1] - p1[1]*p2[0]), dot_product(p1[0], p1[1], p2[0], p2[1]))) * (180 / pi);
}

function is_point_inside_triangle(triangle, point) {
	return point_in_triangle(point[0], point[1],
...
function triangulate(polygon) {
	var final_triangles = [];
	var vertices = array_clone(polygon);
	var original_vertices = array_clone(polygon);
	var triangles_found = -1;
	
	while (triangles_found != 0) {
		triangles_found = 0;
		
		for (var i = 0; i < len(vertices); i++) {
			
			var prev_vertice = vertices[(i-1+len(vertices))%len(vertices)];
			var next_vertice = vertices[(i+1)%len(vertices)];
			var vertice = vertices[i];
			
			//get vector from prev vertice to vertice
			var vector1 = [vertice[0] - prev_vertice[0], vertice[1] - prev_vertice[1]];
			//other one
			var vector2 = [next_vertice[0] - vertice[0], next_vertice[1] - vertice[1]];
			//internal angle
			...
			if (angle < 180) {
				...
				//vertices that arent part of triangle
				var points = [];
				for (var j = 0; j < len(original_vertices); j++) {
				    var p = original_vertices[j];
				    if !(array_contains(triangle, p)) {
				        array_push(points, p);
				    }
				}
				//check if vertice inside triangle
				var inside_evaluation = [];
				for (var j = 0; j < len(points); j++) {
					array_push(inside_evaluation, is_point_inside_triangle(triangle, points[j]));
				}
				//check if no pointo in triangulos
				if !(array_contains(inside_evaluation, true)) {
					//push triangle to end
					array_push(final_triangles, triangle);
					//remove vertice from vertices
					array_delete(vertices, i, 1);
					//MORE TRIANGLLES
					triangles_found++;
...
function snap(keyword) { //snap dspecific keyword and data
	if (file_exists("savedgame.save")) {
		//make into data mode
		var struct = pull();
...
function get_keys() { //print list of all keys
	if (file_exists("savedgame.save")) {
		var struct = pull();
	} else {
		return [];
	...
	var final = variable_struct_get_names(struct);
	array_sort(final, true);
	return final;
}

function in_save(keyword) { //check if keyword is already present
	if (file_exists("savedgame.save")) {
		var struct = pull();
