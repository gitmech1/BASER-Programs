...
function circle_circle(x1, y1, r1, x2, y2, r2) {
	if (pythag(x1, y1, x2, y2) < r1+r2) {
		return true;
	}
	return false;
}
...
function ease_in_out(n){ //eases in and out
	return 3*power(n, 2) - 2*power(n, 3);
}

function ease_in_out_cubic(n){ //eases in and out but MORE
	if (n < 0.5) {
